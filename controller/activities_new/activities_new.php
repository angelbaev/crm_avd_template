<?php
include_once(DIR_MODEL."activities".SLASH."activities_new.php");

class ControllerActivities_new extends Controller {
	private $activities;
	private $activity;
	private $doc_id;
    ...
    {  
    	parent::__construct();
    	$this->template = 'activities_new/activities.php';	
		...
		if ($page_tab == "edit") { 
			...
			switch ($this->act) {
				...
				//	$this->contract_id = $this->model->updateSupplier($this->contract_id, $data);
					...
					$this->data["action"] = "index.php?route=activities_new&tab=edit&token=".get_token()."&act=update&doc_id=".$this->doc_id;
					break;
				case "edit":
					$this->data["action"] = "index.php?route=activities_new&tab=edit&token=".get_token()."&act=edit&doc_id=".$this->doc_id;
					break;
				case "cancel":
					$this->data["action"] = "index.php?route=activities_new&tab=edit&token=".get_token()."&act=edit";
					break;
				case "delete":
					$this->data["action"] = "index.php?route=activities_new&tab=all&token=".get_token()."&act=list";
					break;
				case "all":
					default:
					$this->data["action"] = "index.php?route=activities_new&tab=all&token=".get_token()."&act=edit";
					break;
			}
			...
			switch ($this->act) {
				case "edit":
					$this->data["action"] = "index.php?route=activities_new&tab=edit&token=".get_token()."&act=all";
					break;
				case "delete":
					$this->model->deteleSupplier($this->contract_id);
					$this->data["action"] = "index.php?route=activities_new&tab=owner&token=".get_token()."&act=list";
					break;
				case "all":
					default:
					$this->data["action"] = "index.php?route=activities_new&tab=owner&token=".get_token()."&act=list";
