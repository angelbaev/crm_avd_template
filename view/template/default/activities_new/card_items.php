<?php
function selectStatus($name, $val) {
	$html = "<select name=\"".$name."\">";
	foreach($GLOBALS["CARD_STATUS"] as $id => $value){
		$sel = $id == $val? "selected":"";
		$html .= "<option value=\"".$id."\" ".$sel.">".$value."</value>";
	}
	$html .= "</select>";
	return $html;
}
...
?>		
				...
				<tr id="c_status_row_<?php echo $id; ?>" class="t-card">
					<td class="label">Статус</td>
					<td>
						<?php echo selectStatus("card[".$id."][card_status]", ($item["card_status"]?CARD_STATUS_F:CARD_STATUS_W)); ?>
					</td>
					<td colspan=3>&nbsp;</td>
				</tr>
...
?>		
				...
				<tr id="s_status_row_<?php echo $id; ?>" class="t-card">
					<td class="label">Статус</td>
					<td>
						<?php echo selectStatus("supplier[".$id."][CStatus]", ($item["CStatus"]?CARD_STATUS_F:CARD_STATUS_W)); ?>
					</td>
					<td colspan=3>&nbsp;</td>
				</tr>
...
function addCardItem() {
	html = ''+
		...
		'	<td colspan=4 class="label"><span class="t-cart_title">Дейност</span></td>'+
		'	<td><a class="remove" href="javascript: void(0);" onclick="removeCardItem(\''+startCIndex+'\')"></a></td>'+
		'</tr>'+
		...
		'		<td>&nbsp;</td>'+
		'	</tr>'+
		'	<tr id="c_status_row_'+startCIndex+'" class="t-card">'+
		'		<td class="label">Статус</td>'+
		'		<td><?php echo selectStatus("card['+startCIndex+'][card_status]", CARD_STATUS_W); ?></td>'+
		'		<td colspan=3>&nbsp;</td>'+
		'	</tr>'+
...
function addSupplierItem () {
	html = ''+
		...
		'	<td colspan=4 class="label"><span class="t-cart_title">Контрагент</span></td>'+
		'	<td><a class="remove" href="javascript: void(0);" onclick="removeSupplierItem(\''+startCIndex+'\')"></a></td>'+
		'</tr>'+
		...
		'		<td>&nbsp;</td>'+
		'	</tr>'+
		'	<tr id="s_status_row_'+startSIndex+'" class="t-card">'+
		'		<td class="label">Статус</td>'+
		'		<td><?php echo selectStatus("supplier['+startSIndex+'][CStatus]", CARD_STATUS_W); ?></td>'+
		'		<td colspan=3>&nbsp;</td>'+
		'	</tr>'+
		'	<tr id="s_save_row_'+startCIndex+'" class="t-card">'+
		'	  <td colspan="5" style="text-align: right;"><input type="button" onclick="saveSupplierItem(\''+startCIndex+'\', \'\');" class="submit_button button_add" value="Запис" name="add_position"></td>'+
		'	</tr>';
