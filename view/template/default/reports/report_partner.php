		<script type="text/javascript">
			...
			function report_partner() {
	  		...
	  		if ($('select[name=\'report_partner_id\'] option:selected').val() != '*') {
	  			url += '&report_partner_id=' + encodeURIComponent($('select[name=\'report_partner_id\'] option:selected').val());
				}
				/*
...
<form action="<?php echo $action;?>" name="fEdit" method="post">
		<table width="100%" class="form" border="0" cellspacing="0" cellpadding="2">
				...
				<tr>
					...
					<td>
						<select name="report_partner_id"  class="ie240">
							<option value="*"> - Избери клиент - </option>
							<?php if (count($all_partners)) { ?>
							<?php foreach ($all_partners as $pid => $item) { ?>
							<option value="<?php echo $pid; ?>" <?php echo ($pid == $report_filter["report_partner_id"]? " selected":""); ?>> <?php echo $item["PName"];?> </option>
							<?php } ?>
							<?php } ?>
						</select>
					</td>
  ...
  <script type="text/javascript">
  	...
  	/*
	  		...
	  		if ($('select[name=\'report_partner_id\'] option:selected').val() != '*') {
					$('select[name=\'report_partner_id\']').css({'border':'1px red solid'});
				}
	  		if ($('select[name=\'report_order_type\'] option:selected').val() != '*') {
...
<table class="list" border="1" cellspacing="0" cellpadding="1">
	<colgroup>
		...
		<col width="110"><!--Приета-->
		<col width="110"><!--За издаване-->
