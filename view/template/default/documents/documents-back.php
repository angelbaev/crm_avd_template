...
<form action="<?php echo $action;?>" name="fEdit" method="post">
		<table width="100%" class="form" border="0" cellspacing="0" cellpadding="2">
				...
				<tr>
					<td class="label">Клиент</td>
					<td>
					 <input type="text" id="partner_id" class="ie240" value="">
					 <input type="hidden" id="partner_id_h" name="doc[partner_id]" value="<?php echo $document["partner_id"]; ?>">
					 <!--
						<select id="partner_id" name="doc[partner_id]"  class="ie240s">
							<option value=""> - Избери клиент - </option>
							<?php if (count($all_partners)) { ?>
...
#docModal .modalClose:hover,
#docModal .modalClose:focus {
    color: #d65f05 ;
...
#docModal .modal-body {padding: 2px 16px;
  height: 400px;
  overflow: auto;
}
...
.partner-list li.selected {
  background-color: rgb(255,204,204);
	...
	<?php 
		...
		";
		$js_partners = array();
		foreach ($all_partners as $pid => $item) {
		  $name = str_replace("'", "", $item["PName"]);
			$js_partners[] = "{id: ".$pid.", name: '".$name."'}";
		}
		
		if (count($js_partners)) {
			print "var PARTNERS = [".implode(",", $js_partners)."];";
		}
		print "
    ...
    <div class="modal-body">
      <table>
        <tr>
          ...
          <td><input type="text" id="search_partner" class="ie240" value="" onkeyup="search(this.value);"></td>
        </tr>
      </table>
      <ul class="partner-list" id="partnerList">
        <script type="text/javascript">
          for(var i in PARTNERS) {
            document.writeln('<li id="pid_'+PARTNERS[i]['id']+'" onclick="selectPartner(\''+PARTNERS[i]['id']+'\', \''+i+'\');">'+PARTNERS[i]['name']+'</li>');
          }
        </script>
      </ul>
    </div>
...
var btn = document.getElementById("partner_id");
...
btn.onclick = function() {
    ...
    $('.partner-list li').removeClass('selected');
    var partner_id = document.getElementById("partner_id_h").value;
    if (partner_id.length > 0) {
      document.getElementById('pid_' + partner_id).className = 'selected';
    }
}
  ...
  if (value.length > 0) {
    for(var i in PARTNERS) {
      ...
      if (name.toLowerCase().startsWith(value.toLowerCase())) {
        items.push(PARTNERS[i]);
      }
    }
  }
  reloadList(items);
}
function selectPartner(pid, index) {
  document.getElementById('partner_id_h').value = pid;
  document.getElementById('partner_id').value = PARTNERS[index]['name'];
  
  modal.style.display = "none";
}

function selectPartnerSearch(pid, index) {
  document.getElementById('partner_id_h').value = pid;
  document.getElementById('partner_id').value = items[index]['name'];
  
  modal.style.display = "none";
}
function reloadList(items) {
  document.getElementById('partnerList').innerHTML = '';
  for(var i in items) {
    document.getElementById('partnerList').innerHTML += '<li id="pid_'+items[i]['id']+'" onclick="selectPartnerSearch(\''+items[i]['id']+'\', '+i+');">'+items[i]['name']+'</li>';
  }
}
</script>